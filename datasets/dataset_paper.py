...
class ChexpertTestDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        info = self.data_to_iterate[idx] # Accesss the metadata information for the image at index idx
        image_path = os.path.join(self.source,'images',info['filename']) 
...
class BratsMetTestDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(n_slices-1):
            ...
            slice = PIL.Image.fromarray(slice).convert("RGB").resize((self.args.image_size,self.args.image_size),PIL.Image.Resampling.BILINEAR)
            slices.append(slice)  

            if info.get("mask", None): 
                mask =  mask_scan.get_fdata()[:, :, _]
                mask = PIL.Image.fromarray(mask).convert("L").resize((self.args.image_size,self.args.image_size),PIL.Image.Resampling.NEAREST)
                mask = np.array(mask).astype(float)/255.0
                mask [mask!=0.0] = 1.0
            else:
                mask = np.zeros((self.args.image_size,self.args.image_size), dtype=float)
        ...
        slices = [self.transform_img(slice).unsqueeze(0) for slice in slices] # transform to be digested by openCLIP
        slices = torch.cat(slices, dim = 0)
        masks = torch.cat([torch.from_numpy(mask).unsqueeze(0) for mask in masks], dim = 0)
